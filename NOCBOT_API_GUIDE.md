...
### `GET /api/ont/<ont_id>/services`

List every service provisioned on an ONT. **DB-only — no live SMX
fallback** (unlike `GET /api/ont/<ont_id>` above): only checks the local
`Service` table, so it never needs SMX credentials and never pays an SMX
round trip. If an ONT you expect to see isn't showing services, hit
`GET /api/ont/<ont_id>` first (or wait for the next SMX sync) to get it
cached, then retry this endpoint. An ONT can have zero, one, or several
services — the response is always a list.
...
- `200` — ONT found in the DB; `services` may be an empty list if the ONT
  has none provisioned:
  ...
  ```json
  {
    ...
    "services": [
      {
        "service_id": "3bd7a2c8-21a7-4c3d-99a6-b1fbb2e0210e",
        "ont_id": "H01-01-01",
        "olt_id": "AR1OLT01",
        "port_id": "g2",
        "port_description": "40036002",
        "service_description": "40036002",
        "service_name": "Ooredoo_SuperNet_Unlimited_30M",
        "policy_map": "Ooredoo_PM_HSI_UNTAG",
        "s_vlan": 101,
        "c_vlan": 842,
        "ingress": 409600,
        "egress": 409600,
        "admin_state": "enabled",
        "provisioned_state": "Provisioned Success",
        "subscriber_id": "H01-01-01",
        "session_state": "",
        "last_updated": "2026-07-24 23:05:49",
        "account_number": ""
      }
    ]
  }
  ...
  `source` is always `"database"` here (no SMX fallback).

- `400` — invalid `ont_id` format: `{"error": "Invalid ont_id format"}`

- `401` — missing/invalid API key (see Authentication above)

- `404` — the ONT itself isn't found in the database:
  `{"error": "ONT '<id>' not found in database"}` (note: this is about
  the *ONT*, not the services — an ONT that exists but has no services
  still returns `200` with `"services": []`)
...
```bash
curl -H "X-API-Key: prod-key-abc123" \
  http://localhost:5000/api/ont/H01-01-01/services
```

### `GET /api/services/search`

Find service(s) by **exact** port description (case-insensitive) — useful
when you have a port/order/circuit reference (e.g. `40036002`, `BB18020454`)
but not the ONT ID. **DB-only — no live SMX fallback**: only checks the
local `Service` table, so it never needs SMX credentials and never pays an
SMX round trip. If a service you expect to see doesn't turn up, it likely
hasn't synced into the DB yet.
...
|---------------------|----------|----------------------------------------------------------|
| `port_description`  | Yes      | Exact port description to match. Alphanumeric, `_` and `-` only. |

**Responses**

- `200` — one or more matching services found (same service object shape as
  `/api/ont/<ont_id>/services`; can be more than one row if multiple
  services share that exact port description in the DB):
    ...
    "services": [
      {
        ...
        "port_description": "40036002",
        "service_name": "Ooredoo_SuperNet_Unlimited_30M",
        "...": "remaining Service table columns"
      }
    ]
  }
  ```
...
- `400` — missing/invalid `port_description`:
  `{"error": "port_description query parameter is required and must be alphanumeric (with _ and -)"}`

- `401` — missing/invalid API key (see Authentication above)

- `404` — no service in the database has that port description:
  `{"error": "No service found matching port description '<value>'"}`
...
```bash
curl -H "X-API-Key: prod-key-abc123" \
  "http://localhost:5000/api/services/search?port_description=40036002"
```
...
recorded by `Library/APIRequestLog.py`, written by an `after_request` hook
on `api_bp` (`routes/api.py`) using the `request.api_auth_*` attributes
`Library/APIAuth.py` sets during the auth check. Unauthenticated dashboard
endpoints (`/api/data`, `/api/lots`, `/api/health`, etc.) never set those
attributes, so they're not logged here.

**Storage:** a JSON-lines file at `logs/api_requests.jsonl` — deliberately
*not* the main SQLite DB (`Databases/BOTDB.db`), so high-volume request
traffic never bloats production tables or DB backups. The file is wiped and
restarted from empty automatically once its oldest entry turns 24h old (checked
on every write, see `_reset_if_stale()`), so it's a rolling last-24h window,
not a durable long-term log. `logs/` is already gitignored. For anything
that needs to survive longer than a day, use the DB-table approach instead
(or export the file periodically) — this is intentionally short-lived
operational data, not an audit trail.
...
Writes go through `log_request()`; reads (for the admin page) go through
`get_page()` / `get_known_labels()` — both read and parse the whole file
each call, which is fine given it's capped to <=24h of traffic.
