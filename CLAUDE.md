...
- **Dhiraagu browser sidecar (CDP).** In Docker, Dhiraagu's Cloudflare-bound headed Chrome runs in a dedicated `dhiraagu-browser` service (same image, `browser-entrypoint.sh`) instead of inside the app. It launches ONE real Chrome under Xvfb+fluxbox with a persistent `/profile` volume; a `socat` forwarder exposes CDP on `:9223` (internal only) and noVNC on `:6080`. The app drives it via `playwright.chromium.connect_over_cdp` (set by `DHIRAAGU_BROWSER_CDP=dhiraagu-browser:9223`); when that env is set the app skips its own Xvfb/noVNC. One Chrome, one profile — automation (CDP) and the human (noVNC) share the same window, so there's no `SingletonLock` conflict. The sidecar **must run on the same host** so Cloudflare sees the same egress IP (`cf_clearance` is IP-bound). Launched directly (not via Playwright), so there's no `--enable-automation` flag and `navigator.webdriver` stays false. The other 3 portals still launch locally/headless in the app.

### Dhiraagu browser sidecar — internals

- `browser-entrypoint.sh`: clears stale X locks → Xvfb `:99` → fluxbox → x11vnc+noVNC → clears stale Chrome `Singleton*` in `/profile` → starts `socat TCP-LISTEN:9223,fork → 127.0.0.1:9222` → `exec google-chrome … --remote-debugging-port=9222 --remote-allow-origins=* --user-data-dir=/profile`. Chrome is PID 1, so if it dies the container restarts it.
- **Why socat:** Chrome binds DevTools to **loopback only** and ignores `--remote-debugging-address=0.0.0.0` (recent-Chrome security restriction), so a cross-container connect to `:9222` is refused (`ECONNREFUSED`). socat exposes `0.0.0.0:9223 → 127.0.0.1:9222`; the app connects to `:9223`.
- App side: `BaseExtractor.cdp_endpoint()` (Dhiraagu reads `DHIRAAGU_BROWSER_CDP`); when set, `_get_or_create_browser()` builds `BrowserManager(cdp_endpoint=...)` whose `start()` calls `_start_cdp()` → connects **by resolved IP** (Chrome's DevTools HTTP endpoint 403s a hostname `Host` header; it also builds its ws URL from that `Host`, so IP+`:9223` works end-to-end), reuses the remote `contexts[0]`/page. `stop()` only **disconnects** (never closes the remote browser). Memory PID detection is skipped (remote); the sidecar bounds its own memory.
- Auto-pause/auto-resume for manual CF still works over CDP (Dhiraagu keeps `FORCE_HEADED = True`, which is what flags a persistent challenge for the pause). Local dev with `DHIRAAGU_BROWSER_CDP` unset falls back to launching headed Chrome locally as before.
