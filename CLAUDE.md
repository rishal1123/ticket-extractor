...
- **Dhiraagu browser sidecar (CDP).** In Docker, Dhiraagu's Cloudflare-bound headed Chrome runs in a dedicated `dhiraagu-browser` service (same image, `browser-entrypoint.sh`) instead of inside the app. It launches ONE real Chrome under Xvfb+fluxbox with `--remote-debugging-port=9222 --remote-allow-origins=*` and a persistent `/profile` volume, exposing CDP on `:9222` (internal only) and noVNC on `:6080`. The app drives it via `playwright.chromium.connect_over_cdp` (set by `DHIRAAGU_BROWSER_CDP=dhiraagu-browser:9222`); when that env is set the app skips its own Xvfb/noVNC. One Chrome, one profile — automation (CDP) and the human (noVNC) share the same window, so there's no `SingletonLock` conflict. The sidecar **must run on the same host** so Cloudflare sees the same egress IP (`cf_clearance` is IP-bound). Launched directly (not via Playwright), so there's no `--enable-automation` flag and `navigator.webdriver` stays false. The other 3 portals still launch locally/headless in the app.

### Dhiraagu browser sidecar — internals

- `browser-entrypoint.sh`: clears stale X locks → Xvfb `:99` → fluxbox → x11vnc+noVNC → clears stale Chrome `Singleton*` in `/profile` → `exec google-chrome … --remote-debugging-port=9222 --remote-debugging-address=0.0.0.0 --remote-allow-origins=* --user-data-dir=/profile`. Chrome is PID 1, so if it dies the container restarts it.
- App side: `BaseExtractor.cdp_endpoint()` (Dhiraagu reads `DHIRAAGU_BROWSER_CDP`); when set, `_get_or_create_browser()` builds `BrowserManager(cdp_endpoint=...)` whose `start()` calls `_start_cdp()` → connects **by resolved IP** (Chrome's DevTools HTTP endpoint 403s a hostname `Host` header), reuses the remote `contexts[0]`/page. `stop()` only **disconnects** (never closes the remote browser). Memory PID detection is skipped (remote); the sidecar bounds its own memory.
